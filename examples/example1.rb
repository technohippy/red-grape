...
puts "g.v(1).out('knows').filter{it.age < 30}: #{g.v(1).out('knows').filter{it.age < 30}}"
puts "g.v(1).out('knows').filter{it.age < 30}.name: #{g.v(1).out('knows').filter{it.age < 30}.name}"
puts "g.v(1).out('knows').filter{it.age < 30}.name.transform{it.size}: #{g.v(1).out('knows').filter{it.age < 30}.name.transform{it.size}}"
puts "g.v(1).out('knows').filter{it.age < 30}.name.transform{it.size}.to_a: #{g.v(1).out('knows').filter{it.age < 30}.name.transform{it.size}.to_a}"

puts "g.v(1).side_effect{@x = it}: #{g.v(1).side_effect{@x = it}}"
puts "g.v(1).side_effect{@x = it}.out('created'): #{g.v(1).side_effect{@x = it}.out('created')}"
...
#puts "g.v(1).out('knows').if_then_else(->{it < 30}, ->{it.name}, ->{it.out('created').name}): #{g.v(1).out('knows').if_then_else(->{it < 30}, ->{it.name}, ->{it.out('created').name})}"
